...
score qr = sizeVersionCase (scoreMV qr) (scoreV qr)

scoreV :: QR -> Int -> Int
scoreV t s = negate $ 
               sum [sameColorRow t s (i,0) 0 True | i<-[0..s-1]] + 
               sum [sameColorColumn t s (0,i) 0 True | i<-[0..s-1]] +
               sum [block t (i,j) | i<-[0..s-2], j<-[i..s-2]] + 
               sum [pattern t (i,j) | i<-[0..s-12], j<-[0..s-12]] + 
               proportion t s

scoreMV :: Table Bool -> Int -> Int
scoreMV t s = (min sr sl)*16 + (max sr sl)
   where sr = sum [if t ! (n,s-1) then 1 else 0 | n<-[0..s-1]]
         sl = sum [if t ! (s-1,n) then 1 else 0 | n<-[0..s-1]]
