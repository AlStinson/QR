...
sizeVersionCase1 :: (Int -> a) -> (Int -> a) -> Version -> a
sizeVersionCase1 f g = versionCase (f . h . size) (g . h . size)
   where h x = x-1
...
unSize x | x<=17     = if and [mMV==0,1<=dMV,4 >=dMV] 
                       then Just $ MV dMV else Nothing
         | otherwise = if and [mV ==0,1<=dV ,40>=dV ] 
                       then Just $ V  dV  else Nothing 
   where (dMV,mMV) = divMod (x-9) 2
         (dV,mV) = divMod (x-17) 4

unsafeUnSize :: Int -> Version 
unsafeUnSize x = if x<=17 then MV $ div (x-9) 2 else V $ div (x-17) 4
